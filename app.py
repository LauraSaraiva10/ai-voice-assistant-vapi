...
DATABASE_URL = 'sqlite:///./Voice_Assistant_Database.db'
        ...
        if tool_call.function.name == 'addReminder':
            ...
            return {
                'results': [
                    {
                        'toolCallId': tool_call.id,
                        'result': ReminderResponse.model_validate(reminder).model_dump()
                    }
                ]
            }
...
def delete_todo(request: VapiRequest, db: Session = Depends(get_db)):
    for tool_call in request.message.toolCalls:
        if tool_call.function.name == 'deleteReminder':
            ...
            reminder: Reminder = db.query(Reminder).filter(Reminder.id == reminder_id).first()
        ...
        if tool_call.function.name == 'addCalendarEntry':
            ...
            return {
                'results': [
                    {
                        'toolCallId': tool_call.id,
                        'result': CalendarEventResponse.model_validate(calendar_event).model_dump()
                    }
                ]
            }
...
def delete_todo(request: VapiRequest, db: Session = Depends(get_db)):
    for tool_call in request.message.toolCalls:
        if tool_call.function.name == 'deleteCalendarEntry':
            ...
            event: CalendarEvent = db.query(CalendarEvent).filter(CalendarEvent.id == event_id).first()
